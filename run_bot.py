...
async def init_db():
    async with aiosqlite.connect(
        f"{os.path.realpath(os.path.dirname(__file__))}/discord_bot/database/database.db"
    ) as db:
        with open(
            f"{os.path.realpath(os.path.dirname(__file__))}/discord_bot/database/schema.sql"
        ) as file:
...
async def load_cogs() -> None:
    ...
    """
    for file in os.listdir(f"{os.path.realpath(os.path.dirname(__file__))}/discord_bot/cogs"):
        if file.endswith(".py"):
            extension = file[:-3]
            try:
